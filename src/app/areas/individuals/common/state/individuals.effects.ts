...
} from './actions/load-individual.actions';
import {
  LoadIndividuals,
  LoadIndividualsSuccess,
} from './actions/load-individuals.actions';
import { SetCurrentIndividualId } from './actions/set-current-individual-id.actions';
import {
  UpsertIndividual,
  UpsertIndividualSuccess,
} from './actions/upsert-individual.actions';
...
export class IndividualsEffects {
  ...
  setCurrentIndividuaId$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.appStore.select(selectRouteParam('individualId')).pipe(
          map((val) =>
            SetCurrentIndividualId({
  ...
  load$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((ind) =>
        this.dataService.load$(ind.id).pipe(
          map((entry) =>
            LoadIndividualSuccess({
              data: entry,
            })
  ...
  upsert$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((act) =>
        this.dataService.upsert$(act.data).pipe(
          map((entry) =>
            UpsertIndividualSuccess({
              data: entry,
            })
